...
const register = async (req, res) => {
  try {
    ...
    if (check) {
      return res.status(StatusCodes.BAD_REQUEST).json({
        message:
          "this email address already exists, try a different email address",
      });
    }
...
const validateResetCode = async (req, res) => {
  try {
    ...
    return res.status(StatusCodes.OK).json({ message: "You shall pass" });
  } catch (error) {
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ message: error.message });
...
const updateDetails = async (req, res) => {
  try {
    ...
    const updated = await User.findByIdAndUpdate(
      ...
      {
        new: true, // i always need to use new:true with findByIdAndUpdate
      }
    );
...
const follow = async (req, res) => {
  try {
    if (req.user.id !== req.params.id) {
      ...
      return res.status(StatusCodes.BAD_REQUEST).json({
        message:
          "You can't follow yourself, (in real life always follow yourself :))",
      });
    }
  } catch (error) {
    res
...
const deleteRequest = async (req, res) => {
  try {
    if (req.user.id !== req.params.id) {
      ...
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ message: "You can't delete yourself fool!" });
    }
  } catch (error) {
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
